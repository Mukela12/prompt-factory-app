...
- Dev commands default `PROMPTFACTORY_STATE_DIR` to `~/.t3/dev` to keep dev state isolated from desktop/prod state.
...
Set `PROMPTFACTORY_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `PROMPTFACTORY_DEV_INSTANCE`)
- Example: `PROMPTFACTORY_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `PROMPTFACTORY_PORT_OFFSET` to a numeric offset.
