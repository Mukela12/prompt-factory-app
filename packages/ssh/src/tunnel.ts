...
import { extractJsonObject, fromLenientJson } from "@prompt-factory/shared/schemaJson";
import { satisfiesSemverRange } from "@prompt-factory/shared/semver";
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env PROMPTFACTORY_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
...
>()("@prompt-factory/ssh/SshEnvironmentManager") {
