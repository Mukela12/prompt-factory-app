...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@prompt-factory/ssh/SshPasswordPrompt",
) {
...
# Invoked by ssh via SSH_ASKPASS when Prompt Factory re-runs ssh with a cached password
...
printf 'Prompt Factory ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Prompt Factory re-runs\r
...
[Console]::Error.WriteLine("Prompt Factory ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
