...
export const DEFAULT_PROMPT_FACTORY_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".promptfactory"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@prompt-factory/contracts",
    "--filter=@prompt-factory/web",
    "--filter=t3",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=t3"],
  "dev:web": ["run", "dev", "--filter=@prompt-factory/web"],
  "dev:desktop": ["run", "dev", "--filter=@prompt-factory/desktop", "--filter=@prompt-factory/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("PROMPTFACTORY_PORT_OFFSET"),
  devInstance: optionalStringConfig("PROMPTFACTORY_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid PROMPTFACTORY_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `PROMPTFACTORY_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric PROMPTFACTORY_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed PROMPTFACTORY_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_PROMPT_FACTORY_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      PROMPTFACTORY_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.PROMPTFACTORY_PORT = String(serverPort);
      ...
      delete output.PROMPTFACTORY_MODE;
      delete output.PROMPTFACTORY_NO_BROWSER;
      delete output.PROMPTFACTORY_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.PROMPTFACTORY_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.PROMPTFACTORY_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.PROMPTFACTORY_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.PROMPTFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.PROMPTFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.PROMPTFACTORY_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.PROMPTFACTORY_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.PROMPTFACTORY_MODE = "web";
      delete output.PROMPTFACTORY_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read PROMPTFACTORY_PORT_OFFSET/PROMPTFACTORY_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.PROMPTFACTORY_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.PROMPTFACTORY_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Prompt Factory data (equivalent to PROMPTFACTORY_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("PROMPTFACTORY_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to PROMPTFACTORY_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("PROMPTFACTORY_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to PROMPTFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("PROMPTFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to PROMPTFACTORY_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("PROMPTFACTORY_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to PROMPTFACTORY_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("PROMPTFACTORY_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to PROMPTFACTORY_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("PROMPTFACTORY_PORT")),
  ),
