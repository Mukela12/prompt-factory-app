...
export const createOxlintRuleHarness = (ruleName: string): RuleHarness => {
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "prompt-factory-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-prompt-factory", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "prompt-factory", specifier: pluginPath }],
