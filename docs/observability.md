...
Prompt Factory has one server-side observability model:
...
```bash
export PROMPTFACTORY_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export PROMPTFACTORY_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export PROMPTFACTORY_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export PROMPTFACTORY_TRACE_MIN_LEVEL=Info
export PROMPTFACTORY_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `PROMPTFACTORY_OTLP_*`.

macOS app bundle example:

```bash
PROMPTFACTORY_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
PROMPTFACTORY_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
PROMPTFACTORY_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/Prompt Factory.app/Contents/MacOS/Prompt Factory"
```
...
```bash
tail -f "$PROMPTFACTORY_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$PROMPTFACTORY_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$PROMPTFACTORY_HOME/userdata/logs/server.trace.ndjson"
```
...
- `PROMPTFACTORY_OTLP_TRACES_URL` was not set
...
- `PROMPTFACTORY_TRACE_FILE`: override trace file path
- `PROMPTFACTORY_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `PROMPTFACTORY_TRACE_MAX_FILES`: rotated file count, default `10`
- `PROMPTFACTORY_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `PROMPTFACTORY_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `PROMPTFACTORY_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `PROMPTFACTORY_OTLP_TRACES_URL`: OTLP trace endpoint
- `PROMPTFACTORY_OTLP_METRICS_URL`: OTLP metric endpoint
- `PROMPTFACTORY_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `PROMPTFACTORY_OTLP_SERVICE_NAME`: service name, default `t3-server`
