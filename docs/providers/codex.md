...
This guide is for people who want to use more than one Codex account in Prompt Factory.
...
In Prompt Factory Settings, name it something obvious:
...
In Prompt Factory Settings, add another Codex provider:
...
Prompt Factory shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Prompt Factory treats it
