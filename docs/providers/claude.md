...
This guide is for people who want to use more than one Claude setup in Prompt Factory.
...
In Prompt Factory Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means Prompt Factory uses your normal home directory.
...
Then add another Claude provider in Prompt Factory:
...
Prompt Factory only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Prompt Factory keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Prompt Factory Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Prompt Factory stores the value as a server secret and does not
...
Prompt Factory does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Prompt Factory.
