    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          PROMPTFACTORY_PORT: "9999",
          PROMPTFACTORY_MODE: "desktop",
          PROMPTFACTORY_DESKTOP_LAN_HOST: "192.168.1.50",
  ...
  it.effect("resolves backend start config with a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.PROMPTFACTORY_PORT);
        assert.isUndefined(first.env.PROMPTFACTORY_MODE);
        assert.isUndefined(first.env.PROMPTFACTORY_DESKTOP_LAN_HOST);
