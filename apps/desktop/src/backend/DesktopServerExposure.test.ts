...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return makeDesktopEnvironmentLayer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ PROMPTFACTORY_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { PROMPTFACTORY_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        PROMPTFACTORY_DESKTOP_LAN_HOST: "10.0.0.7",
        PROMPTFACTORY_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
