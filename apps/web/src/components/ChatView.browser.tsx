...
import { scopedThreadKey, scopeThreadRef } from "@prompt-factory/client-runtime";
import { createModelCapabilities, createModelSelection } from "@prompt-factory/shared/model";
...
import { DEFAULT_CLIENT_SETTINGS } from "@prompt-factory/contracts/settings";
...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.prompt-factory-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              PROMPTFACTORY_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.PROMPTFACTORY_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("prompt-factory:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              PROMPTFACTORY_PROJECT_ROOT: "/repo/project",
              PROMPTFACTORY_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/Mukela12/prompt-factory-app/pull/1359",
              baseBranch: "main",
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    localStorage.setItem(
      "prompt-factory:client-settings:v1",
      JSON.stringify({
        ...DEFAULT_CLIENT_SETTINGS,
    ...
    try {
      ...
      localStorage.removeItem("prompt-factory:client-settings:v1");
      await mounted.cleanup();
    }
  });
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: Prompt Factory Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
