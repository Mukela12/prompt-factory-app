...
import {
  ...
  setupProjectScript,
} from "@prompt-factory/shared/projectScripts";
...
describe("projectScripts helpers", () => {
  ...
  it("allows overriding runtime env values", () => {
    const env = projectScriptRuntimeEnv({
      project: { cwd: "/repo" },
      extraEnv: {
        PROMPTFACTORY_PROJECT_ROOT: "/custom-root",
        CUSTOM_FLAG: "1",
      },
    });

    expect(env.PROMPTFACTORY_PROJECT_ROOT).toBe("/custom-root");
    expect(env.CUSTOM_FLAG).toBe("1");
    expect(env.PROMPTFACTORY_WORKTREE_PATH).toBeUndefined();
  });
