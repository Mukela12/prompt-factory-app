...
import { deepMerge } from "@prompt-factory/shared/Struct";
import { createModelCapabilities } from "@prompt-factory/shared/model";
import { applyServerSettingsPatch } from "@prompt-factory/shared/serverSettings";
...
process.env.PROMPTFACTORY_CURSOR_ENABLED = "1";
    ...
    describe("ProviderRegistryLive", () => {
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Prompt Factory settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Prompt Factory settings.");
        }),
      );
    });
