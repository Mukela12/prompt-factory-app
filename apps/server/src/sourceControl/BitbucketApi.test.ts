...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/Mukela12/prompt-factory-app/pull-requests/42",
    },
  },
  source: {
  ...
  destination: {
    branch: { name: "main" },
    repository: {
      full_name: "Mukela12/prompt-factory-app",
...
const repositoryJson = {
  full_name: "Mukela12/prompt-factory-app",
  links: {
    html: { href: "https://bitbucket.org/Mukela12/prompt-factory-app" },
    clone: [
      { name: "https", href: "https://bitbucket.org/Mukela12/prompt-factory-app.git" },
      { name: "ssh", href: "git@bitbucket.org:Mukela12/prompt-factory-app.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriverShape["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:Mukela12/prompt-factory-app.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:Mukela12/prompt-factory-app.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            PROMPTFACTORY_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            PROMPTFACTORY_BITBUCKET_EMAIL: "user@example.com",
            PROMPTFACTORY_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/Mukela12/prompt-factory-app/pull-requests/42",
      baseRefName: "main",
    ...
    assert.strictEqual(
      ...
      "https://api.test.local/2.0/repositories/Mukela12/prompt-factory-app/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "Mukela12/prompt-factory-app" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/Mukela12/prompt-factory-app/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    const cloneUrls = yield* bitbucket.getRepositoryCloneUrls({
      cwd: "/repo",
      repository: "Mukela12/prompt-factory-app",
    });
    const defaultBranch = yield* bitbucket.getDefaultBranch({ cwd: "/repo" });

    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "Mukela12/prompt-factory-app",
      url: "https://bitbucket.org/Mukela12/prompt-factory-app.git",
      sshUrl: "git@bitbucket.org:Mukela12/prompt-factory-app.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/Mukela12/prompt-factory-app",
          "https://api.test.local/2.0/repositories/Mukela12/prompt-factory-app/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/Mukela12/prompt-factory-app");
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:Mukela12/prompt-factory-app.git",
      },
      reference: "42",
      force: true,
    });
