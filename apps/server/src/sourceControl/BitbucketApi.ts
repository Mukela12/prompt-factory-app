...
import { sanitizeBranchFragment } from "@prompt-factory/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@prompt-factory/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("PROMPTFACTORY_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("PROMPTFACTORY_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("PROMPTFACTORY_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("PROMPTFACTORY_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
export class BitbucketApi extends Context.Service<BitbucketApi, BitbucketApiShape>()(
  "prompt-factory/source-control/BitbucketApi",
) {}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set PROMPTFACTORY_BITBUCKET_EMAIL and PROMPTFACTORY_BITBUCKET_API_TOKEN, or PROMPTFACTORY_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
