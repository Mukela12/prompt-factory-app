...
it.layer(
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      setEnv("PORT", "5173");
      setEnv("PROMPTFACTORY_PORT", "3773");
      ...
      try {
        ...
        expect(spawnInput.env.PROMPTFACTORY_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            PROMPTFACTORY_PROJECT_ROOT: "/repo",
            PROMPTFACTORY_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.PROMPTFACTORY_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.PROMPTFACTORY_WORKTREE_PATH, "/repo/worktree-a");
